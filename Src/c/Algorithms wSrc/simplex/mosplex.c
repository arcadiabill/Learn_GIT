...
int skip_comments(FILE *fp) {
  int ch, num_comments=0;
  while( (ch = looking_at(fp)) == '#' ) { // start of comment?
    num_comments++;
    while( ch != '\n' && ch != EOF) {
      ch = fgetc(fp);
      putchar(ch);
...
void read_tableau(Tableau *tab, const char * filename) {
  ...
  for(i=0;i<tab->m; i++) {
    for(j=0;j<tab->n; j++) {
      ...
      if( err == 0) { // if nothing read, might be a comment
        if (skip_comments(fp)) {
           j--; // retry reading same element.
           continue;
        }
      }
      if (err == EOF) {
