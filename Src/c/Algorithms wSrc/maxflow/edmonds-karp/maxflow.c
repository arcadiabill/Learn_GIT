/*
  from http://puzzless60.googlecode.com/svn-history/r24/trunk/src.puzzles/maxflow.c
  Edmonds-Karp algorithm for finding a maximum flow and minimum
  cut in a network.

  Almost identical to the Ford-Fulkerson algorithm,
  but using breadth-first search to find the
  _shortest_ augmenting path is a good way to guarantee
  termination and ensure the time complexity is not dependent on
  the actual value of the maximum flow.
  */
...
void write_gv(int *edges, int *capacity, int *flow, int *cut, int source, int sink,
  int ne, int nv, int ret ){
  FILE *mg;
  int i;
  printf("Writing maxflow.gv\n");
  mg = fopen("maxflow.gv", "w");
  if(!mg) printf("Cannot write maxflow.gv\n"), exit(-1);
  fprintf(mg, "%s",
      "// View this maxflow graph in Graphviz\n"
      "digraph G {\n"
      "  layout=dot;\n"
      "  splines=true;\n");
  fprintf(mg,"  %d [color=pink label = \"%d (source) maxflow=%d\"];\n",
      source, source, ret);
  fprintf(mg, "  %d [color=pink label = \"%d (sink)\"];\n", sink, sink);

  for (i = 0; i < nv; i++) {
    if (i==source || i==sink) continue;
    fprintf(mg, "  %d [label = \"%d\" color=%s ];\n",
      i, i, (cut[i]? "red /*mincut*/":"blue"));
  }

  for(i = 0; i < ne; i++){ // forall edges
    int tight = flow[i] == capacity[i];
    fprintf(mg, "  %d -> %d [label = \"%d/%d\" color=%s ];\n",
        edges[2*i], edges[2*i+1], flow[i], capacity[i], 
          tight? "green /*full*/":"black /*spare*/");
  }
...
int main(void) {
    ...
    j = ne;
    capacity[ne] = rand()%10; ADDEDGE(p+0,q+0);
    capacity[ne] = rand()%10; ADDEDGE(p+1,q+0);
    capacity[ne] = rand()%10; ADDEDGE(p+1,q+1);
    capacity[ne] = rand()%10; ADDEDGE(p+2,q+1);
    capacity[ne] = rand()%10; ADDEDGE(p+2,q+2);
    capacity[ne] = rand()%10; ADDEDGE(p+3,q+2);
    capacity[ne] = rand()%10; ADDEDGE(p+3,q+3);
    capacity[ne] = rand()%10; ADDEDGE(p+4,q+3);
    /* capacity[ne] = rand()%5; ADDEDGE(p+2,q+4); */
    ...
    for (i = 0; i < ne; i++)
        printf("flow %d: edge(%d -> %d)\n", flow[i], edges[2*i], edges[2*i+1]);

    printf("difficult set = { ", i);
    for (i = 0; i < nv; i++) {
        if (cut[i] == 0)
            printf("%d, ", i);
    }
    ...
    write_gv(edges, capacity, flow, cut, source, sink, ne, nv, ret);
