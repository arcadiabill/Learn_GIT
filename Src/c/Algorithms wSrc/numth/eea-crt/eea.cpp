// Date: 2012-Dec-22 Sat 16:03
// also see ~/cc/math/eea.c
...
int main(int argc, char *argv[]){
  ...
  if (argc > 2) { // usage: cmd a b ...
    ...
    for(a=9;a<17;a+=1) {
      for(b=12;b<16;b+=1) {
        eea_out res;
        // printf("Calculating eea_rec(%d,%d):\n", a, b);
        res = eea_rec(a,b);
        // printf("(%-2d * %2d) + (%2d * %2d) = %-2d = gcd(%2d,%2d)",
        //   res.x, a, res.y, b, res.gcd, a, b);
        if (res.gcd==1) {
          printf("inv(%d) is %d mod %d\n", a, res.x, b);
          printf("ie. %2d * %2d = 1 mod %d", a, res.x, b);
        }
        printf("\n");
      }
    }
