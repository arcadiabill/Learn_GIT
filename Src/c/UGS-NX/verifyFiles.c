...
#include <stdio.h>
#include <string.h>
#include <stdlib.h>          // defines _MAX_PATH (260 chars)
...
verifyFiles(FILE *ptr_in, char *sourcePath, char *destPath, int required)
{
   char Line[_MAX_PATH + 1];
	...
	while (fgets(Line, _MAX_PATH, ptr_in) != NULL)
	{
      ...
      strncat(sourceFile, sourcePath, _MAX_PATH);
		strncat(sourceFile, Line      , _MAX_PATH);

		strncat(destFile, destPath, _MAX_PATH);
		strncat(destFile, Line    , _MAX_PATH);
